...
# ADS1x15 Class
#
# Originally written by K. Townsend, Adafruit (
# https://github.com/adafruit/Adafruit-Raspberry-Pi-Python-Code/tree/master
# /Adafruit_ADS1x15)
...
class ADS1x15:
    ...
    __ADS1015_REG_CONFIG_OS_SINGLE = 0x8000  # Write: Set to start a
    # single-conversion
    __ADS1015_REG_CONFIG_OS_BUSY = 0x0000  # Read: Bit = 0 when conversion
    # is in progress
    __ADS1015_REG_CONFIG_OS_NOTBUSY = 0x8000  # Read: Bit = 1 when device is
    #  not performing a conversion

    __ADS1015_REG_CONFIG_MUX_MASK = 0x7000
    __ADS1015_REG_CONFIG_MUX_DIFF_0_1 = 0x0000  # Differential P = AIN0,
    # N = AIN1 (default)
    __ADS1015_REG_CONFIG_MUX_DIFF_0_3 = 0x1000  # Differential P = AIN0,
    # N = AIN3
    __ADS1015_REG_CONFIG_MUX_DIFF_1_3 = 0x2000  # Differential P = AIN1,
    # N = AIN3
    __ADS1015_REG_CONFIG_MUX_DIFF_2_3 = 0x3000  # Differential P = AIN2,
    # N = AIN3
    ...
    __ADS1015_REG_CONFIG_MODE_SINGLE = 0x0100  # Power-down single-shot mode
    #  (default)
    ...
    __ADS1015_REG_CONFIG_DR_1600SPS = 0x0080  # 1600 samples per second (
    # default)
    ...
    __ADS1015_REG_CONFIG_CMODE_TRAD = 0x0000  # Traditional comparator with
    # hysteresis (default)
    __ADS1015_REG_CONFIG_CMODE_WINDOW = 0x0010  # Window comparator

    __ADS1015_REG_CONFIG_CPOL_MASK = 0x0008
    __ADS1015_REG_CONFIG_CPOL_ACTVLOW = 0x0000  # ALERT/RDY pin is low when
    # active (default)
    __ADS1015_REG_CONFIG_CPOL_ACTVHI = 0x0008  # ALERT/RDY pin is high when
    # active

    __ADS1015_REG_CONFIG_CLAT_MASK = 0x0004  # Determines if ALERT/RDY pin
    # latches once asserted
    __ADS1015_REG_CONFIG_CLAT_NONLAT = 0x0000  # Non-latching comparator (
    # default)
    __ADS1015_REG_CONFIG_CLAT_LATCH = 0x0004  # Latching comparator

    __ADS1015_REG_CONFIG_CQUE_MASK = 0x0003
    __ADS1015_REG_CONFIG_CQUE_1CONV = 0x0000  # Assert ALERT/RDY after one
    # conversions
    __ADS1015_REG_CONFIG_CQUE_2CONV = 0x0001  # Assert ALERT/RDY after two
    # conversions
    __ADS1015_REG_CONFIG_CQUE_4CONV = 0x0002  # Assert ALERT/RDY after four
    # conversions
    __ADS1015_REG_CONFIG_CQUE_NONE = 0x0003  # Disable the comparator and
    # put ALERT/RDY in high state (default)

    # Dictionaries with the sampling speed values
    # These simplify and clean the code (avoid the abuse of if/elif/else
    # clauses)
    spsADS1115 = {
    ...
    def readRaw(self, channel=0, pga=6144, sps=250):
        ...
        # If sps is in the dictionary (defined in init) it returns the value
        # of the constant othewise it returns the value for 250sps. This
        # saves a lot of if/elif/else code!
        if self.ic == self.__IC_ADS1015:
            config |= self.spsADS1015.setdefault(sps,
    ...
    def readADCSingleEnded(self, channel=0, pga=6144, sps=250):
        ...
        The sample rate for this mode (single-shot) can be used to lower the
        noise \
        (low sps) or to lower the power consumption (high sps) by duty
        cycling, \
        ...
        # If sps is in the dictionary (defined in init) it returns the value
        #  of the constant
        # othewise it returns the value for 250sps. This saves a lot of
        # if/elif/else code!
        if self.ic == self.__IC_ADS1015:
            config |= self.spsADS1015.setdefault(sps,
                                                 self.__ADS1015_REG_CONFIG_DR_1600SPS)
    ...
    def readADCDifferential(self, chP=0, chN=1, pga=6144, sps=250):
        ...
        The sample rate for this mode (single-shot) can be used to lower the
        noise (low sps) or to lower the power consumption (high sps) by duty
        cycling, see data sheet page 14 for more info. The pga must be given
        ...
        if self.ic == self.__IC_ADS1015:
            # Shift right 4 bits for the 12-bit ADS1015 and convert to mV
            return (
                   ((result[0] << 8) | (result[1] & 0xFF)) >> 4) * pga / 2048.0
        else:
            # Return a mV value for the ADS1115
            # (Take signed values into account as well)
            val = (result[0] << 8) | (result[1])
    ...
    def startContinuousConversion(self, channel=0, pga=6144, sps=250):
        """Starts the continuous conversion mode and returns the first ADC
        reading \
        in mV from the specified channel. \
        The sps controls the sample rate. \
        The pga must be given in mV, see datasheet page 13 for the supported
          values. \
    ...
    def startContinuousDifferentialConversion(self, chP=0, chN=1, pga=6144,
                                              sps=250):
        """Starts the continuous differential conversion mode and returns the
        first ADC reading \
    ...
    def startSingleEndedComparator(self, channel, thresholdHigh,
                                   thresholdLow, \
                                   pga=6144, sps=250, \
                                   activeLow=True, traditionalMode=True,
                                   latching=False, \
                                   numReadings=1):
        """Starts the comparator mode on the specified channel, see datasheet
        pg. 15. \
        In traditional mode it alerts (ALERT pin will go low)  when voltage
        exceeds  \
        thresholdHigh until it falls below thresholdLow (both given in mV). \
        In window mode (traditionalMode=False) it alerts when voltage
        doesn't lie\
        between both thresholds.\
        In latching mode the alert will continue until the conversion value ' \
             'is read. \
        numReadings controls how many readings are necessary to trigger an ' \
             'alert: 1, 2 or 4.\
        Use getLastConversionResults() to read the current value  (which may ' \
             'differ \
        from the one that triggered the alert) and clear the alert pin in ' \
             'latching mode. \
        This function starts the continuous conversion mode.  The sps ' \
             'controls \
        ...
        # Write config register to the ADC
        # Once we write the ADC will convert continously and alert when
        # things happen,
        # we can read the converted values using getLastConversionResult
        bytes = [(config >> 8) & 0xFF, config & 0xFF]
        self.i2c.writeList(self.__ADS1015_REG_POINTER_CONFIG, bytes)

    def startDifferentialComparator(self, chP, chN, thresholdHigh,
                                    thresholdLow, \
                                    pga=6144, sps=250, \
                                    activeLow=True, traditionalMode=True,
                                    latching=False, \
